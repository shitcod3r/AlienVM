...
public:
	...
	AlienVirtualMachine(void* _commands, size_t _commands_size)
	{
		...
		vm.AX = 0;
		vm.EXIT = 0;
		vm.data = (uint32_t*)calloc(0x80, 1);
		vm.ZF = 0;
		vm.SP = 0;
...
private:
	...
	void status()
	{
		...
		for (int i = 0; i < 0x80 / 4; i++)
		{
			if (i % 16 == 0)
				printf("0x%02x: ", i);
			printf("%04x%s", vm.data[i], ((i + 1) % 16 == 0 ? "\n" : " "));
		}
	...
	char* decode()
	{
		...
		case 0x0a:
			sprintf(decoded, "PUSH\tMEM[%02x]\n\t\t   PUSH\t%02x", vm.COMMANDS[vm.PC + 1], vm.data[vm.COMMANDS[vm.PC + 1]]);
			break;
		...
		case 0x11:
			sprintf(decoded, "JE\tMEM[%02x]  MEM[%02x]  ->  CODE[%02x]\n\t\t   JE\t%02x\t %02x\t  ->  CODE[%02x]",
				...
				vm.data[vm.COMMANDS[vm.PC + 1]], vm.data[vm.COMMANDS[vm.PC + 2]], ((int16_t)vm.COMMANDS[vm.PC + 3]) * 6);
			break;
		...
		case 0x15:
			sprintf(decoded, "XOR\tMEM[%02x]  MEM[%02x]  ->   MEM[%02x]\n\t\t   XOR\t%02x\t %02x\t  ->   MEM[%02x]",
				...
				vm.COMMANDS[vm.PC + 1], vm.data[vm.COMMANDS[vm.PC + 2]], vm.data[vm.COMMANDS[vm.PC + 3]], vm.COMMANDS[vm.PC + 1]);
			break;

		case 0x16:
			sprintf(decoded, "STORE\tCODE[MEM[%02x]]  MEM[%02x]\n\t\t   STORE\tCODE[%02x]      %02x",
				vm.COMMANDS[vm.PC + 1], vm.COMMANDS[vm.PC + 2], *(int32_t*)(vm.data + vm.COMMANDS[vm.PC + 1]),
				vm.data[vm.COMMANDS[vm.PC + 2]]);
			break;

		case 0x17:
			sprintf(decoded, "LOAD\tMEM[%02x]  CODE[MEM[%02x]]\n\t\t   LOAD\tMEM[%02x]\t CODE[%02x]\n\t\t   LOAD\tMEM[%02x]  %02x",
				vm.COMMANDS[vm.PC + 1], vm.COMMANDS[vm.PC + 2],
				vm.COMMANDS[vm.PC + 1], vm.data[vm.COMMANDS[vm.PC + 2]],
				vm.COMMANDS[vm.PC + 1], vm.COMMANDS[vm.data[vm.COMMANDS[vm.PC + 2]]]);
			break;
	...
	void add() // 0x00
	{
		vm.data[vm.COMMANDS[vm.PC + 1]] = vm.COMMANDS[vm.PC + 2] + vm.COMMANDS[vm.PC + 3];
	...
	void addi() // 0x01
	{
		vm.data[vm.COMMANDS[vm.PC + 1]] = vm.data[vm.COMMANDS[vm.PC + 2]] + vm.COMMANDS[vm.PC + 3];
	...
	void jmp() // 0x08
	{
		vm.PC = vm.data[vm.COMMANDS[vm.PC + 2]];
		vm.PC += 6;
	}
	...
	void push() // 0x0a
	{
		vm.STACK[vm.SP++] = vm.data[vm.COMMANDS[vm.PC + 1]];
	...
	void pop() // 0x0b
	{
		vm.data[vm.COMMANDS[vm.PC + 1]] = vm.STACK[--vm.SP];
	...
	void mov() // 0x0c
	{
		//vm.data[vm.COMMANDS[vm.PC + 1]] = (uint32_t)vm.COMMANDS[vm.PC + 2];
	...
	void je() // 0x11
	{
		if (vm.data[vm.COMMANDS[vm.PC + 1]] == vm.data[vm.COMMANDS[vm.PC + 2]])
		{
	...
	void xorr() // 0x15
	{
		vm.data[vm.COMMANDS[vm.PC + 1]] = vm.data[vm.COMMANDS[vm.PC + 2]] ^ vm.data[vm.COMMANDS[vm.PC + 3]];
	...
	void store() // 0x16
	{
		vm.COMMANDS[*(int32_t*)(vm.data + vm.COMMANDS[vm.PC + 1])] = vm.data[vm.COMMANDS[vm.PC + 2]];
	...
	void load() // 0x17
	{
		vm.data[vm.COMMANDS[vm.PC + 1]] = vm.COMMANDS[*(int32_t*)(vm.data + vm.COMMANDS[vm.PC + 2])];
